...
@Component({
  ...
  template: `
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-1">
      ...
      <button
        type="button"
        class="flex items-center justify-center gap-2 px-6 py-3 text-sm font-medium text-[--color-text]
               bg-[--color-surface] border-2 border-[--color-border] rounded-lg
               hover:bg-[--color-surface-alt] hover:border-[--color-text-muted]
               active:scale-95 transition-all duration-200 ease-soft focus:outline-none
               focus:ring-2 focus:ring-black/10"
